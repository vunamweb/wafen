...
require_once 'Customweb/PayEngine3/Method/Validator/CreditCard.php';
 ...
 */
class Customweb_PayEngine3_Method_CreditCard extends Customweb_PayEngine3_Method_Default {
	...
	public function getJavaScriptCallbackFunction(Customweb_PayEngine3_Authorization_Transaction $transaction){
		...
		if ($transaction->getTransactionContext()->getAlias() instanceof Customweb_PayEngine3_Authorization_Transaction) {
			$url = $this->getContainer()->createSecuredEndpointUrl('process', 'alias', $transaction);
			...
			return "function(formFieldValues){($successCallback)('$url&payengine3-tracking=' + formFieldValues['payengine3-tracking']);}";
		}
		
		$failCallback = $context->getJavaScriptFailedCallbackFunction();
		$language = $transaction->getTransactionContext()->getOrderContext()->getLanguage()->getIso2LetterCode();
		$url = "'" . $this->getContainer()->createSecuredEndpointUrl('process', 'return', $transaction);
	...
	public function getVisibleFormFields(Customweb_Payment_Authorization_IOrderContext $orderContext, $aliasTransaction, $failedTransaction, Customweb_Payment_Authorization_IPaymentCustomerContext $paymentCustomerContext){
		...
		$control->addValidator(new Customweb_PayEngine3_Method_Validator_CreditCard($control, $this->getWidgetPrefix()));
