...
class PayEngine3Cw_OrderContext extends PayEngine3Cw_AbstractOrderContext
{
	...
	public function __construct(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod, array $order_info, array $order_totals) {
		...
		foreach ($this->productData as $key => $data) {
			$this->productData[$key]['total'] = PayEngine3Cw_Util::convertTo($data['quantity'] * $data['total'], $this->getCurrencyCode());
			$this->productData[$key]['total_with_tax'] = PayEngine3Cw_Util::convertTo($data['quantity'] * $data['total_with_tax'], $this->getCurrencyCode());
		}
		...
		// Langauge
		$this->languageCode = PayEngine3Cw_Language::getLanguageCodeByLanguageId($this->orderInfo['language_id']);

		// Order total
		$this->orderTotal = PayEngine3Cw_Util::convertTo($this->orderInfo['total'], $this->getCurrencyCode());
