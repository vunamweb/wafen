...
require_once 'PayEngine3Cw/AbstractOrderContext.php';
 ...
 *
 */
class PayEngine3Cw_SessionOrderContext extends PayEngine3Cw_AbstractOrderContext
{
	...
	public function __construct(Customweb_Payment_Authorization_IPaymentMethod $paymentMethod, $registry) {
		...
		if (isset($session->data['shipping_address']) && is_array($session->data['shipping_address'])) {
			$shipping = PayEngine3Cw_Util::mergeArray($shipping, $session->data['shipping_address']);
		}
		if (isset($session->data['shipping']['shipping_address']) && is_array($session->data['shipping']['shipping_address'])) {
			$shipping = PayEngine3Cw_Util::mergeArray($shipping, $session->data['shipping']['shipping_address']);

		}
		if (isset($session->data['guest']['shipping']) && is_array($session->data['guest']['shipping'])) {
			$shipping = PayEngine3Cw_Util::mergeArray($shipping, $session->data['guest']['shipping']);
		}
		...
		if (isset($session->data['guest']['payment']) && is_array($session->data['guest']['payment'])) {
			$payment = PayEngine3Cw_Util::mergeArray($payment, $session->data['guest']['payment']);

		}
		if (isset($session->data['payment_address']) && is_array($session->data['payment_address'])) {
			$payment = PayEngine3Cw_Util::mergeArray($payment, $session->data['payment_address']);
		}
		...
		// Order Totals
		$this->orderTotals = PayEngine3Cw_Util::getOrderTotals($registry);

		// Langauge
		$this->languageCode = PayEngine3Cw_Language::getCurrentLanguageCode();
