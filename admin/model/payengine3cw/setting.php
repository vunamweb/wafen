...
class ModelPayEngine3CwSetting extends ModelPayEngine3CwAbstractSetting
{

	public function render(PayEngine3Cw_SettingApi $api) {
		$this->api = $api;
	...
	protected function renderElement($item, $controlHtml, $storeId) {
		...
		if (PayEngine3Cw_Store::DEFAULT_STORE_ID != $storeId) {
			$defaultValue = $this->api->getValue($item['key'], PayEngine3Cw_Store::DEFAULT_STORE_ID);
			if ($defaultValue == $item['value']) {
				$isDefault = true;
				$formGroupClasses .= ' payengine3cw-use-default';
			}
		}
		...
		$output .= '<label class="col-sm-2 control-label" for="' . $this->getControlId($item, $storeId) . '">
				...
				title="" data-original-title="' . PayEngine3Cw_Language::_($item['description']) . '">' . PayEngine3Cw_Language::_($item['title']) . '</span></label>';
		
		// Add default checkbox
		if (PayEngine3Cw_Store::DEFAULT_STORE_ID != $storeId) {
			
			$checked = '';
			$classes = 'payengine3cw-control-box';
			if ($isDefault) {
			...
			$output .= '<div class="col-sm-10 ' . $classes . '">' . $controlHtml . '</div>
				<div class="col-sm-offset-2 col-sm-10">
					<div class="checkbox">
						<label class="payengine3cw-default-box">
							<input type="checkbox" id="' . $id .'"  name="use_default[' . $item['key'] . '][' . $storeId . ']" value="true" ' . $checked . ' />
							' .  PayEngine3Cw_Language::_('Use default store value') . '
						</label>
					</div>
				</div>';
		}
	...
	protected function renderOrderStatusSelect(array $item, $storeId) {
		...
		$item['options'] = $item['options'] + PayEngine3Cw_OrderStatus::getOrderStatuses();
	...
	protected function renderFile(array $item, $storeId) {
		...
		$output .= '<label for="reset-setting-' . $item['key'] . '-' . $storeId .'">' . PayEngine3Cw_Language::_('Reset') . '</label></div>';
		
		$stream = $item['value'];
		if ($stream instanceof Customweb_Core_Stream_Input_File) {
			$output .= '<div class="current-file-path">' . PayEngine3Cw_Language::_('File Path: ') . $stream->getFilePath() . '</div>';
		}
