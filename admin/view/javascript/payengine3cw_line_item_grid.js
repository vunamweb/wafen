(function($) {
	...
	var PayEngine3CwLineItemGrid = {
		decimalPlaces: 2,
		currencyCode: 'EUR',
		
		init: function() {
			this.decimalPlaces = parseFloat($("#payengine3cw-decimal-places").val());
			this.currencyCode = $("#payengine3cw-currency-code").val();
			this.attachListeners();
		},
		
		attachListeners: function() {
			$(".payengine3cw-line-item-grid input.line-item-quantity").each(function() {
				PayEngine3CwLineItemGrid.attachListener(this);
			});
			$(".payengine3cw-line-item-grid input.line-item-price-excluding").each(function() {
				PayEngine3CwLineItemGrid.attachListener(this);
			});
			$(".payengine3cw-line-item-grid input.line-item-price-including").each(function() {
				PayEngine3CwLineItemGrid.attachListener(this);
			});
		},
		...
		attachListener: function(element) {
			$(element).change(function() {
				PayEngine3CwLineItemGrid.recalculate(this);
			});
		...
		recalculate: function(eventElement) {
			...
			var row = $('.payengine3cw-line-item-grid tr[data-line-item-index="' + lineItemIndex + '"]');
