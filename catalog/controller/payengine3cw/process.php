...
require_once 'PayEngine3Cw/Adapter/IframeAdapter.php';
...
require_once 'PayEngine3Cw/Adapter/WidgetAdapter.php';


class ControllerPayEngine3CwProcess extends PayEngine3Cw_AbstractController
{
	private function redirectSetCookieIfRequired($action) {
		if($_SERVER['REQUEST_METHOD'] !== 'GET') {
			header_remove('set-cookie');
			header('Location: ' . PayEngine3Cw_Util::getUrl('process', $action, array('cw_transaction_id' => $_REQUEST['cw_transaction_id']), true));
			die();
		}
	}
	...
	public function notify() {
		header_remove('set-cookie');
		$dispatcher = new Customweb_Payment_Endpoint_Dispatcher(PayEngine3Cw_Util::getEndpointAdapter(), PayEngine3Cw_Util::getContainer(), array(
			0 => 'Customweb_PayEngine3',
 			1 => 'Customweb_Payment_Authorization',
 		));
		$response = new Customweb_Core_Http_Response($dispatcher->invokeControllerAction(PayEngine3Cw_HttpRequest::getInstance(), 'process', 'index'));
		$response->send();
		die();
	}
	...
	public function payment_page_update() {
		...
		$this->response->setOutput($this->renderView(PayEngine3Cw_Template::resolveTemplatePath(PayEngine3Cw_Template::PAYMENT_FORM_TEMPLATE), $data));
	}
	...
	private function getCheckoutFormData($failedTransaction) {
		...
		if ($failedTransaction !== null && $failedTransaction instanceof PayEngine3Cw_Entity_Transaction) {
			$paymentMethodName = $failedTransaction->getPaymentMachineName();
		}
		else if (isset($order_info['payment_code'])) {
			$paymentMethodName = str_replace('payengine3cw_', '', $order_info['payment_code']);
		}
		else {
			die("Unable to determine the current selected payment method.");
		}

		$paymentMethod = PayEngine3Cw_PaymentMethod::getPaymentMethod($paymentMethodName);
	...
	public function payment() {
		...
		$data['breadcrumbs'][] = array(
			'text'      => PayEngine3Cw_Language::_("Payment"),
			'href'      => $this->url->link('checkout/checkout', '', 'SSL'),
			'separator' => $this->language->get('text_separator')
		);

		// Translations:
		$data['heading_title'] = PayEngine3Cw_Language::_("Payment");

		$failedTransaction = null;
		if (isset($_REQUEST['failed_transaction_id'])) {
			$failedTransaction = PayEngine3Cw_Entity_Transaction::loadById($_REQUEST['failed_transaction_id']);
		...
		$this->response->setOutput($this->renderView(PayEngine3Cw_Template::resolveTemplatePath('template/payengine3cw/payment_page.tpl'), $data, array(
			'common/column_left',
			'common/column_right',
			'common/content_top',
	...
	public function iframe() {
		...
		$paymentAdapter = PayEngine3Cw_Util::getAuthorizationAdapterFactory()->getAuthorizationAdapterByName($transaction->getTransactionObject()->getAuthorizationMethod());

		$adapter = PayEngine3Cw_Util::getShopAdapterByPaymentAdapter($paymentAdapter);

		if (!($adapter instanceof PayEngine3Cw_Adapter_IframeAdapter)) {
		...
		$adapter->prepareWithFormData(PayEngine3Cw_Util::getFormData(), $transaction);
	...
	public function widget() {
		...
		if (!($adapter instanceof PayEngine3Cw_Adapter_WidgetAdapter)) {
	...
	public function server_authorization() {
		...
		$response = new Customweb_Core_Http_Response($paymentAdapter->processAuthorization($transactionObject, PayEngine3Cw_Util::getFormData()));
		
		PayEngine3Cw_Util::getTransactionHandler()->persistTransactionObject($transactionObject);
		$response->send();
		die();
	}
	...
	public function iframe_breakout() {
		...
		$this->response->setOutput($this->renderView(PayEngine3Cw_Template::resolveTemplatePath('template/payengine3cw/iframe_breakout.tpl'), $data, array(
			'common/column_left',
			'common/column_right',
			'common/content_top',
			'common/content_bottom',
	...
	/**
	 * Redirect the customer to the payment page of the Concardis PayEngine 3.0.
	 */
	public function redirection() {

		$transaction = $this->getTransactionFromRequest();

		$adapter = PayEngine3Cw_Util::getAuthorizationAdapterFactory()->getAuthorizationAdapterByName($transaction->getTransactionObject()->getAuthorizationMethod());
		...
		$params = PayEngine3Cw_Util::getFormData();
		...
		if ($headerRedirection) {
			...
			$data['button_continue'] = PayEngine3Cw_Language::_("Continue");
			PayEngine3Cw_Util::getEntityManager()->persist($transaction);

			$this->template = PayEngine3Cw_Template::resolveTemplatePath('template/payengine3cw/redirect.tpl');
			...
			$this->response->setOutput($this->renderView(PayEngine3Cw_Template::resolveTemplatePath('template/payengine3cw/redirect.tpl'), $data, array(
				'common/column_left',
				'common/column_right',
				'common/content_top',
				'common/content_bottom',
	...
	public function failed() {
		...
		$url = PayEngine3Cw_Util::getUrl('checkout', '', array('failed_transaction_id' => $failedTransaction->getTransactionId()), true, 'checkout');
		header('Location: ' . $url);
		die();
	...
	public function success() {
		...
		while (true) {
			...
			$transaction = PayEngine3Cw_Entity_Transaction::loadById(intval($_REQUEST['cw_transaction_id']), false);

			$transactionObject = $transaction->getTransactionObject();

			$url = null;
			if ($transactionObject->isAuthorizationFailed()) {
				$url = PayEngine3Cw_Util::getUrl('process', 'failed', array('cw_transaction_id' => $_REQUEST['cw_transaction_id']), true);
			}
			else if ($transactionObject->isAuthorized()) {
				$url = PayEngine3Cw_Util::getUrl('success', '', array(), true, 'checkout');
			}
			...
			if (time() - $start > $maxExecutionTime) {
				$url = PayEngine3Cw_Util::getUrl('process', 'timeout', array('cw_transaction_id' => $_REQUEST['cw_transaction_id']), true);
				header('Location: ' . $url);
				die();
			}
			else {
	...
	public function timeout() {
		...
		$data['timeout_message'] = PayEngine3Cw_Language::_('It seems as your order was successful. However we do not get any feedback from the payment processor. Please contact us to find out what is going on with your order.');
		$data['order_message'] = PayEngine3Cw_Language::_('Please mention the following order id:');
		$data['order_id'] = $transaction->getOrderId();
		
		
		$this->response->setOutput($this->renderView(PayEngine3Cw_Template::resolveTemplatePath('template/payengine3cw/timeout.tpl'), $data, array(
			'common/column_left',
			'common/column_right',
			'common/content_top',
			'common/content_bottom',
	...
	private function getTransactionFromRequest() {
		...
		$transaction = PayEngine3Cw_Entity_Transaction::loadById(intval($_REQUEST['cw_transaction_id']));
