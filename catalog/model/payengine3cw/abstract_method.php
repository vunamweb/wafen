...
require_once 'PayEngine3Cw/SessionOrderContext.php';
...
abstract class ModelPaymentPayEngine3CwAbstract extends Model implements PayEngine3Cw_IPaymentMethodDefinition
{
	/**
	...
	public function __construct($registry) {
		...
		PayEngine3Cw_Util::setRegistry($registry);
	}
	
	public function getMethod($address, $total)
	{
		
		$this->paymentMethod = new PayEngine3Cw_PaymentMethod($this);
		...
		if ($this->isEnabled($address, $total)) {
			...
			$method_data = array(
				'code'       => 'payengine3cw_' . strtolower($this->paymentMethod->getPaymentMethodName()),
	...
	protected function validate() {
		$orderContext = new PayEngine3Cw_SessionOrderContext(new PayEngine3Cw_PaymentMethodWrapper($this->paymentMethod), $this->registry);
		$adapter = PayEngine3Cw_Util::getAuthorizationAdapterFactory()->getAuthorizationAdapterByContext($orderContext);
		$customerContext =  PayEngine3Cw_Util::getPaymentCustomerContext($orderContext->getCustomerId());
		try {
			$adapter->preValidate($orderContext, $customerContext);
			PayEngine3Cw_Util::persistPaymentCustomerContext($customerContext);
			return true;
		}
		catch(PayEngine3Cw_IncompleteDataException $e) {
			...
			PayEngine3Cw_Util::persistPaymentCustomerContext($customerContext);
			PayEngine3Cw_Util::log("Validation failed with: " . $e->getMessage(), 'error');
			return true;
		}
		catch(Exception $e) {
			PayEngine3Cw_Util::persistPaymentCustomerContext($customerContext);
